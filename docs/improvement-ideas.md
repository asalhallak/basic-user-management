...
| Register page while logged out | ~~Register form visible without JWT~~ | Fixed — `RegisterComponent` redirects to login on init when no session; auth layout no longer redirects all signed-in users away from `/account/register` |
...
| Tests | Karma/Jasmine configured; `extractHttpErrorMessage`, `JwtInterceptor`, `ErrorInterceptor`, `AuthGuard`, `HomeComponent`, `LoginComponent`, `RegisterComponent`, `AddEditComponent`, `ListComponent`, `AlertComponent`, `AlertService`, `AuthController.Login`, `UsersController` CRUD/not-found/conflict mapping, `AuthService.Login`, `JwtHelper.GenerateToken`, `UsersService` CRUD/duplicate-login, HTTP integration tests (auth + users CRUD via `WebApplicationFactory` + in-memory EF Core), `AccountService.isLoggedIn`, `AccountService.login`, `AccountService.logout`, `AccountService.register`, `AccountService.update`, `AccountService.getById`, `AccountService.getAll`, and `AccountService.delete` unit tests added | Add more API integration scenarios or E2E smoke tests |
...
| Target | Suggestion |
|--------|------------|
| ~~API integration tests~~ | ~~End-to-end controller tests with in-memory EF Core or Testcontainers SQL Server (unit tests mock services today)~~ Fixed — see `UserManagementAPI/UserManagement.API.Tests/ApiIntegrationTests.cs` and `ApiWebApplicationFactory.cs` |
...
**Already covered** (for reference): `extractHttpErrorMessage`, `AuthController.Login`, `AuthService.Login`, `JwtHelper.GenerateToken`, `UsersService`, `UsersController`, HTTP integration tests (login, JWT protection, users CRUD/not-found/conflict via in-memory database), `AccountService.isLoggedIn`, all other `AccountService` HTTP methods, `AuthGuard`, `AlertService`, `JwtInterceptor`, `ErrorInterceptor`, `AppComponent`, auth and users `LayoutComponent`, `HomeComponent`, `LoginComponent`, `RegisterComponent`, `AddEditComponent`, `ListComponent`, and `AlertComponent`. Spec file paths are listed in the README testing section.
