...
| Tests | Karma/Jasmine configured; `extractHttpErrorMessage`, `JwtInterceptor`, `ErrorInterceptor`, `AuthGuard`, `LoginComponent`, `RegisterComponent`, `AddEditComponent`, `AlertService`, `AuthController.Login`, `UsersController` CRUD/not-found/conflict mapping, `AuthService.Login`, `JwtHelper.GenerateToken`, `UsersService` CRUD/duplicate-login, `AccountService.login`, `AccountService.register`, `AccountService.update`, `AccountService.getById`, `AccountService.getAll`, and `AccountService.delete` unit tests added | Add API integration tests or more Angular component tests (e.g. `ListComponent`) |
