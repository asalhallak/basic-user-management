...
`deleteLabel(user)` picks the name shown in the confirmation and on the Delete button's `aria-label`:
...
The Delete button exposes `aria-label="Delete {label}"` so screen readers distinguish rows when multiple Delete buttons appear in the table.
...
| Accessibility | Delete button `aria-label` uses the same `deleteLabel(user)` helper as the confirm dialog |
