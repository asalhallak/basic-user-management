...
Automated coverage is limited but growing: CI and `make ci` run headless Karma/Jasmine unit tests for the Angular `extractHttpErrorMessage` helper, `AccountService.login`, and `AccountService.update`, plus xUnit tests for `AuthService.Login`.
...
| `npm test` | `front-end/` | Karma/Jasmine; includes unit tests for `extractHttpErrorMessage`, `AccountService.login`, and `AccountService.update` (run `npm test -- --watch=false --browsers=ChromeHeadless` in CI-like environments) |
